...
parser.add_argument('--data_type', type=str, default='flureoscent_microsphere')
...
train_X = np.load(os.path.join(args.data_dir, args.data_type, 'train_X.npy')).astype(np.float32) / 65535.0
train_Y32 = np.load(os.path.join(args.data_dir, args.data_type, 'train_Y32_%d.npy' % (gq,))).astype(np.float32) / 32.0
train_Y128 = np.load(os.path.join(args.data_dir, args.data_type, 'train_Y128_%d.npy' % (gq,))).astype(np.float32) / 128.0
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        index = random.randint(0, self.train_XY.shape[0] - 1)
        return self.train_XY[index]
...
log_path = './%s/%s/layer_%d_f_%d_gq_%d.txt' % (args.log_dir, args.data_type, Np, nf, gq)
